  ...
  <div class="auth-page container">
    <div class="auth-card">
      <div class="auth-header">
        ...
        <p>Register a customer account to track orders and save shipping addresses</p>
      </div>
...
const handleRegister = async () => {
  ...
  loading.value = true;
  try {
    await auth.register(name.value, email.value, password.value, phone.value, API);
    // Redirect to profile
    router.push('/profile');
  } catch (err) {
    error.value = err.data?.error || 'Registration failed. Try using a different email.';
  } finally {
    loading.value = false;
  }
};

useHead({ title: 'Create Account — Demo Store' });
</script>

<style scoped>
.auth-page { display: flex; align-items: center; justify-content: center; min-height: 75vh; padding: 2rem 0; }
