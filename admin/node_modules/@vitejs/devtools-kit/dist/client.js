...
function parseRemoteConnection(input) {
	...
	if (!payload || typeof payload !== "object") throw new Error("[vite-devtools-kit] Remote connection descriptor must be an object.");
	const info = payload;
	if (info.v !== 1) throw new Error(`[vite-devtools-kit] Unsupported remote connection descriptor version: ${String(info.v)}`);
	if (info.backend !== "websocket" || typeof info.websocket !== "string" || !info.websocket) throw new Error("[vite-devtools-kit] Remote connection descriptor must carry a websocket URL.");
	if (typeof info.authToken !== "string" || !info.authToken) throw new Error("[vite-devtools-kit] Remote connection descriptor must carry an auth token.");
	if (typeof info.origin !== "string") throw new Error("[vite-devtools-kit] Remote connection descriptor must carry an origin.");
	return info;
}
/**
...
*/
async function connectRemoteDevTools(options = {}) {
	const info = parseRemoteConnection();
	if (!info) throw new Error("[vite-devtools-kit] No remote connection descriptor found in the URL. Open this page through a Vite DevTools dock registered with `remote: true`.");
...
//#endregion
export { CLIENT_CONTEXT_KEY, connectRemoteDevTools, getDevToolsClientContext, parseRemoteConnection };
